...
router.post("/regular", (req, res) => {
  ...
  const userQuery =
    "SELECT firstname, lastname, auto_id AS idNum, email FROM regular_users WHERE auto_id = ?";
  ...
  db.query(userQuery, [user_id], (err, userResults) => {
    ...
    const bookingQuery =
      "INSERT INTO users_bookings (business_id, user_id, firstName, lastName, idNum, email, date, time_slot) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
