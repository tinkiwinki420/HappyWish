...
router.post('/regular', async (req, res) => {
  const { firstname, lastname, id, number, address, dob, email, password } = req.body;
  ...
  emailExists(email, async (err, exists) => {
    ...
    try {
      ...
      db.query(query, [firstname, lastname, id, number, address, dob, email, hashedPassword], (err, results) => {
        if (err) {
          console.error('Database error:', err);
          return res.status(500).json({ message: 'Database error', error: err });
        }
...
router.post('/business', async (req, res) => {
  const { businessName, numOfBusiness, address, email, password, category } = req.body;
  ...
  emailExists(email, async (err, exists) => {
    ...
    try {
      ...
      db.query(query, [businessName, numOfBusiness, address, email, hashedPassword, category], (err, results) => {
        if (err) {
          console.error('Database error:', err);
          return res.status(500).json({ message: 'Database error', error: err });
        }
